...
public class GameManager : MonoBehaviour
{
    ...
    /*public void Load()
    {
        ...
        player2.Link(player1);
        //player1.deck.SetRandom(random);
        //player2.deck.SetRandom(random);
        foreach (Card card in cardCat)
        {
            player1.deck.Insert(card);
        }
        player1.deck.Insert(cardCat[1]);
        player1.deck.Insert(cardCat[1]);
        player1.deck.Insert(cardCat[1]);
        player1.deck.Randomize();
        foreach (Card card in cardCat)
        {
            player2.deck.Insert(card);
        }
        player2.deck.Insert(cardCat[1]);
        player2.deck.Insert(cardCat[1]);
        player2.deck.Insert(cardCat[1]);
        player2.deck.Randomize();
        List<string> vs = new List<string>()
        {
            "Deck List Test",
            player1.deck.DeckAsString(),
            player2.deck.DeckAsString()
        };
        textFile.WriteDataToFile(vs, "DeckList");
    }
